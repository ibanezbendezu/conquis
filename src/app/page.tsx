...
export default function Home() {
  ...
  return (
      <main className={`min-h-screen p-4 sm:p-6 md:p-12 font-sans transition-colors duration-200 ${isDark ? "bg-neutral-950 text-neutral-100" : "bg-neutral-50 text-neutral-900"}`}>        <div className="max-w-6xl mx-auto">
                  ...
                  <div className="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-center gap-3 w-full lg:w-auto">
                      ...
                      <div className="relative flex items-center w-full sm:w-auto">
                          ...
                          <select
                              ...
                              onChange={(e) => { setSelectedCategory(e.target.value); setIsMobileFiltersOpen(false); }} /* Se oculta al seleccionar */
                      ...
                      <div className="relative flex items-center w-full sm:w-auto">
                          ...
                          <select
                              value={sortOrder}
                              onChange={(e) => { setSortOrder(e.target.value as SortOption); setIsMobileFiltersOpen(false); }} /* Se oculta al ordenar */
                              className={`w-full sm:w-auto pl-9 pr-8 py-3 md:py-2 appearance-none border rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-neutral-600 cursor-pointer ${
                                  isDark ? "bg-neutral-900 border-neutral-700 text-neutral-200" : "bg-neutral-50 border-neutral-200 text-neutral-700"
                              }`}
                          >
                      ...
                      <div className={`hidden sm:flex items-center self-start sm:self-auto border rounded-lg p-0.5 ml-0 lg:ml-auto ${isDark ? "border-neutral-700 bg-neutral-900" : "border-neutral-200 bg-neutral-50"}`}>
                          <button onClick={() => setViewMode("grid")} className={`p-1.5 rounded-md transition-colors ${viewMode === "grid" ? (isDark ? "bg-neutral-800 text-neutral-400" : "bg-white text-neutral-700 shadow-sm") : (isDark ? "text-neutral-500 hover:text-neutral-300" : "text-neutral-400 hover:text-neutral-600")}`}><LayoutGrid className="h-4 w-4" /></button>
                          <button onClick={() => setViewMode("list")} className={`p-1.5 rounded-md transition-colors ${viewMode === "list" ? (isDark ? "bg-neutral-800 text-neutral-400" : "bg-white text-neutral-700 shadow-sm") : (isDark ? "text-neutral-500 hover:text-neutral-300" : "text-neutral-400 hover:text-neutral-600")}`}><ListIcon className="h-4 w-4" /></button>
                      </div>

                  </div>
              </div>

              <div className="mt-5 md:mt-3 pt-4 md:pt-3 border-t border-neutral-200 dark:border-neutral-800 flex flex-col md:flex-row justify-between md:justify-end items-center gap-4">
                  ...
                  <span className={`hidden md:block text-xs font-medium ${isDark ? "text-neutral-500" : "text-neutral-400"}`}>
